...
firsttime = 1;
% boolean to indicate the first loop
bestparticleID = 1;
...
for i = 1:
        Npart % loop on all particles
        rmse = 1/ymean*sqrt(sum((y - fct(x,position(i,:))).^2)/Ndata);
% root-mean-square error of the fit, normalized by the mean
if firsttime == 1
