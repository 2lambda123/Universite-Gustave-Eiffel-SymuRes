% %
    ...
    : Convergence calculation %
      ...
      function and the number of violations.% % Feb 2019 -
    Guilhem Mariotte &Sergio F.A.Batista
...
% Initialize the variables : Temp_Gap = 0;
Temp_NumViolations = 0;

% Loop over all routes to calculate the route utilities : Temp_Utility =
    zeros(1, NumRoutes);
for
  iroute = 1 : NumRoutes i_p = 1;
for
  r = Route(iroute).ResPath Temp_meanV =
      mean(Reservoir(r).MeanSpeed(Temp_StartTimeID
                                  : Temp_EndTimeID));
if Temp_meanV
  > 0 Temp_Utility(iroute) = Temp_Utility(iroute) +
                             Route(iroute).TripLengths(i_p) / Temp_meanV;
else
  Temp_Utility(iroute) = Temp_Utility(iroute) + Route(iroute).TripLengths(i_p) /
...
for od = 1:
                 length(ODmacro) % loop on all OD
                     Temp_demdata = 0;
for
  k = 1 : length(ODmacro(od).Demand) Temp_demdata =
              Temp_demdata + sum(ODmacro(od).Demand(k).Data);
end if Temp_demdata > 0 % if the OD(i, j)
is used at one time at least Temp_RouteIDs = ODmacro(od).RoutesID;
Temp_coeffs = Assignment.NewAssignCoeff(Temp_RouteIDs);
Temp_Utils = Temp_Utility(Temp_RouteIDs);
Temp_minUtil = min(Temp_Utils);
% Temp_Gap =
    Temp_Gap +
    sum(Temp_coeffs.*ODmacro(od).NVehicules.*(Temp_Utils - Temp_minUtil))./
        (ODmacro(od).NVehicules.*Temp_minUtil);
Temp_Gap =
    Temp_Gap + sum(Temp_coeffs.*(Temp_Utils - Temp_minUtil))./ (Temp_minUtil);
end
end
...
is used at one time at least Temp_RouteIDs = ODmacro(od).RoutesID;
Temp_coeffs = Assignment.NewAssignCoeff(Temp_RouteIDs);
Temp_Gap = Temp_Gap + sum(Temp_coeffs.*ODmacro(od).NVehicules.*
                          max([Temp_Utils - Assignment.AL(od) 0]))
                              ./
                          (ODmacro(od).NVehicules.*Assignment.AL(od));
end
end
...
for k = 1:
        ...
        if abs(Assignment.NewAssignCoeff(k) - Assignment.OldAssignCoeff(k)) > Assignment.NumViolationsThreshold
            ...
            % Check the convergence tests : if Assignment.Convergence ==
        1 &&Assignment.PredefRoute == 0 if Assignment.ConvergenceIndicatorID ==
        1 % The user decides to use the Gap criterion only.if Temp_Gap <=
            Assignment.MinimumGap ||
    Temp_iter >= Assignment.MaxIteration Assignment.HasConverged = 1;
end elseif Assignment.ConvergenceIndicatorID ==
        2 %
                The user decides to use the Number of violations criterion
                    only.if Temp_NumViolations /
                NumRoutes <=
            Assignment.NumViolationsTolerance ||
    Temp_iter >= Assignment.MaxIteration Assignment.HasConverged = 1;
end elseif Assignment.ConvergenceIndicatorID ==
        3 % The user decides to use the Gap and the Number of violations
                criteria together.if (Temp_Gap <= Assignment.MinimumGap &&
                                      Temp_NumViolations / NumRoutes <=
                                          Assignment.NumViolationsTolerance) ||
    Temp_iter >= Assignment.MaxIteration Assignment.HasConverged = 1;
end end else Assignment.HasConverged = 1;
end
...
for iroute = 1:
                 NumRoutes % loop on all routes
                 Temp_TT = mean(Route(iroute).TravelTime(Temp_StartTimeID:Temp_EndTimeID));
if Temp_TT
  == 0 Route(iroute).TotalTime = Route(iroute).FreeFlowTravelTime;
elseif Temp_TT > 0 Route(iroute).TotalTime = Temp_TT;
end
end
for r = 1:
        NumRes
...
Reservoir(r).TotalTravelTime = ExperiencedTravelTime(Simulation.Time, Temp_nin,
                                                     Temp_nout);
Reservoir(r).TotalTravelTime(1) = 0;
% TT not defined at t = 0 if acc ini = 0
                                       end
...
% For the path of the i - th vehicle,
    save the reservoir travel time %
        on the corresponding Reservoir.TotaltravelTime structure.i_p = 1;
for
  r = Route(Vehicle(iveh).RouteID).ResPath Reservoir(r).TotalTravelTime(iveh) =
      Temp_vehTT(i_p);
i_p = i_p + 1;
end
end
...
Reservoir(r).TotalTravelTime =
    Reservoir(r).TotalTravelTime(Reservoir(r).TotalTravelTime ~ = Inf);
...
for iroute = 1:
             ...
             Temp_routeTT = 0;
for
  r = Route(iroute).ResPath Temp_routeTT =
      Temp_routeTT + mean(Reservoir(r).TotalTravelTime);
end % Travel time is converted from seconds to hours because of the %
    exponentials in the Logit and Mixed Logit.Route(iroute).TotalTime =
    Temp_routeTT;
end
...
if Simulation
  .Solver == 2 Global.CurrentTimeID = Global.NumEvents + 1;
Snapshot.Global = Global;
Snapshot.Vehicle = Vehicle;
Snapshot.NextEvent = NextEvent;
end

    if Assignment.Periods(Assignment.CurrentPeriodID +
                          1) >= Simulation.Duration % End of the simulation

                                    % Track eventual gridlocks Temp_listres = [
];
for
  r = 1 : NumRes if Simulation.Solver == 1 Temp_acctest =
          sum(sum(Reservoir(r).Acc >= Reservoir(r).MaxAcc));
else Temp_acctest = 0;
end Temp_speedtest = sum(sum(Reservoir(r).MeanSpeed < 0));
if Temp_acctest
  > 0 || Temp_speedtest > 0 Temp_listres = [Temp_listres r];
end
end
