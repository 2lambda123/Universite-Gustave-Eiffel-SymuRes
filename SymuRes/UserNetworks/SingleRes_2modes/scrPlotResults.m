...
Results(iresu).Solver = 1;
% Choice of the solver. 1:
accbased / 2:
tripbased
Results(iresu).Name = 'SC32';
% Simulation name
Results(iresu).Name2 = 'acc-based';
...
Results(iresu).Network = 'SingleRes_2modes';
% Choice of a network defined by user
Results(iresu).Solver = 2;
% Choice of the solver. 1:
accbased / 2:
tripbased
Results(iresu).Name = 'SC21';
% Simulation name
Results(iresu).Name2 = 'trip-based';
% Name to print on the graph legends
...
% final time [s]
Dt_frame = 50;
% time step between two frames [s]
Nframes = floor(tfinal/Dt_frame) + 1;
% number of frames
t_frame = 0:
          Dt_frame:
          tfinal;
...
PrintFig = 0;
% 1:
print the figure (save as pdf in the img folder) / 0:
do not print
    filename = Results(iresu).Name;
% name for printing
...
ResuList = [1];
% list of results
TimeRange = [0 Simulation.Duration];
% [s]
AccRange = [0 1000];
% [veh]
FlowRange = [0 3];
...
PlotResTotalVal = 1;
% 1:
plot reservoir total states / 0:
plot route states only
PlotResInternalDyn = 0;
% 1:
add plots of internal state (debug) / 0:
do not add anything (better when plotting several results)
    Nfig = length(ResList);
% number of subfigures
Nplot = length(RoutesList);
% number of plots per subfigure
Nresu = length(ResuList);
% number of results

% Color and line options
sline = arrayextension(line_perso,Nresu,'column');
...
marginleft = 0.1;
% figure relative left margin
marginbottom = 0.2;
% figure relative bottom margin
intermarginright = 0.15;
% subplot relative right margin
intermarginbottom = 0.05;
% subplot relative bottom margin
intermargintop = 0.2;
% subplot relative top margin
figwidth = 6.5;
% whole figure width [inches]
figheight = Nrow*2.5;
...
% left pos
yi = marginbottom + (Nrow - rowindex(ifig) + intermarginbottom)*(1 - marginbottom)/Nrow;
% bottom pos
w = (1 - intermarginright)*(1 - marginleft)/Ncol;
% width
h = (1 - intermargintop - intermarginbottom)*(1 - marginbottom)/Nrow;
% height
...
% 1:
add free-flow TT / 0:
do not add (better when plotting several results)
    filename = 'test';
% name for printing
Nplot = length(RoutesList);
% number of plots per figure
...
%surf(Temp_N1,Temp_N2,Temp_P1,'EdgeColor','none','FaceAlpha',0.5)
contourf(Temp_N1,Temp_N2,Temp_P1,20,'linestyle','none')
colormap(jet)
end
hold off
caxis(ProdRange1)
%axis([AccRange AccRange])
title(['(' figindex(ifig) ') - \itR_{\rm' int2str(ResList(ifig)) '}'],'FontName',fontname,'FontWeight','Bold','FontSize',FS1)
if ifig + Ncol > Nfig
xlabel('accumulation mode 1 \itn^r \rm[veh]','FontName',fontname,'FontSize',FS)
    end
    if mod(ifig,Ncol) == 1 || Ncol == 1
        ylabel('accumulation mode 2 \itn^r \rm[veh]','FontName',fontname,'FontSize',FS)
        end
        hc = colorbar('east','color','w');
%hc.AxisLocation = 'in';
...
set(gcf,'PaperUnits','inches')
    set(gcf,'PaperSize',[figwidth figheight],'PaperPosition',[0 0 figwidth figheight])
    print('-painters','-dpdf',['UserNetworks/' Results(1).Network '/img/' Results(iresu).Network '_3DMFDmode1_' filename '.pdf'])
    else
        set(gcf,'Position',[10 10 1000 700])
        end


        % 3D-MFD of reservoirs, mode 2
        %--------------------------------------------------------------------------
        figure
        strleg = cellstr(int2str(zeros(Nresu,1)));
hp1 = zeros(1,Nresu);
...
%surf(Temp_N1,Temp_N2,Temp_P2,'EdgeColor','none','FaceAlpha',0.5)
