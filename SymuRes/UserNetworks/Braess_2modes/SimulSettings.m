...
% 1:
the routes are already provided (no assignment) / 0:
calculate the routes according to the OD matrix
...
% Max number of shortest Path that will be considered during the assignment process
Assignment.ShortestPath_Threshold = 0.001;
% This threshold allows to eliminate the less sampled micro path to define the macro path
...
% Model 1:
Aggregation by reservoir
% Model 2:
Aggregation by reservoir and destination
% Model 3:
Aggregation by origin, reservoir and destination
% Model 4:
Aggregation by macro-path
Assignment.TripLengthMethod = 0;
% ID method calculation: 1, 2, 3 or 4
Assignment.LengthStd = 300;
