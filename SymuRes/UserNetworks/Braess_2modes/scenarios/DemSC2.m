% % Scenario 2 : congestion with 2 car and 1 PT routes,
...
% Demand route[R1 R3 R4] %
    ...
    1;
for
  od = 1 : NumODmacro if ODmacro (od).NodeOriginID == 1 &&
       ODmacro(od).NodeDestinationID == 3 od0 = od;
end end Route(iroute).ODmacroID = od0;
...
Route(iroute).Demand0(j).Time = [0 Td1:60:(Td2 + 60)];
% [s] Route(iroute).Demand0(j).Data = [q0 qinD(Td1 : 60 : Td2) q2];
% [veh / s]
...
% [veh / s]
    ...
    -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --MacroNode(
                                                                                                                      ...
                                                                                                                      .Time =
    [0 1000 6000];
MacroNode(9).Capacity.Data = [10 0.5 10];
