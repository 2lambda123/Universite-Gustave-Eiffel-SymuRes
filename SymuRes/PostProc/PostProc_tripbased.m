...
% acc circu(r,t)
Temp_sumavgtriplength = zeros(NumRes,NumTimes);
% sum L(r,t)
Temp_countavgtriplength = zeros(NumRes,NumTimes);
% number of L(r,t)
end
...
% first reservoir in the route of veh i
...
itime = 1;
for i = 1:
        Global.NumEvents % loop on all chronological events
        iveh = Global.VehID(i);
...
else % if the veh is in the middle of a multi-reservoir route:
the current event is an exit and an entry
if Temp_pathindex == 1 % if the veh is actually in the origin reservoir
        ires = o;
% correct ID of the current reservoir
i_r = Route(iroute).ResRouteIndex(ires);
end
while itime < NumTimes && SimulTime(itime) < Vehicle(iveh).ExitTimes(Temp_pathindex)
for r = 1:
...
% total accumulation [veh]
Reservoir(r).AccPerMode = zeros(NumModes,NumTimes);
% accumulation per mode [veh]
Reservoir(r).MeanSpeed = zeros(1,NumTimes);
% mean speed [m/s]
Reservoir(r).MeanSpeedPerMode = zeros(NumModes,NumTimes);
...
Reservoir(r).InflowPerResPerDest = zeros(NumRes,NumRes,NumTimes);
% (adj res, dest res, time)
Reservoir(r).Inflow = zeros(1,NumTimes);
% total effective inflow
Reservoir(r).OutflowPerResPerDest = zeros(NumRes,NumRes,NumTimes);
% (adj res, dest res, time)
Reservoir(r).Outflow = zeros(1,NumTimes);
% total effective outflow
Reservoir(r).NinPerResPerDest = zeros(NumRes,NumRes,NumTimes);
% nin(adj res,d,t)
Reservoir(r).Nin = zeros(1,NumTimes);
% nin(t)
Reservoir(r).NoutPerResPerDest = zeros(NumRes,NumRes,NumTimes);
% nout(adj res,d,t)
Reservoir(r).Nout = zeros(1,NumTimes);
% nout(t)
end

% Inflow and outflow per route
Ni = 10;
% adopt a different sampling to avoid too many oscillations of flows
% Ni = 5;
...
elseif r == j && r ~= i % if Rr is the route destination
                      Temp_prevresID = Temp_path(Reservoir(r).RoutesPathIndex(i_r)-1);
% previous reservoir in the path
Temp_nextresID = 0;
else
    Temp_prevresID = Temp_path(Reservoir(r).RoutesPathIndex(i_r)-1);
% previous reservoir in the path
...
Route(iroute).Demand(Temp_StartTimeID:Temp_EndTimeID) = Route(iroute).ListAssignCoeff(iperiod).*Temp_demOD;
% row vector
end
end
end
