...
% mean speed per mode[m / s] Reservoir(r).AvgTripLength = zeros(1, NumTimes);
% average trip length[m] Reservoir(r).InflowPerResPerDest = zeros(NumRes,
                                                                  NumRes,
                                                                  NumTimes);
% (adj res, dest res, time)Reservoir(r).Inflow = zeros(1, NumTimes);
% total effective inflow[veh / s] Reservoir(r).OutflowPerResPerDest =
    zeros(NumRes, NumRes, NumTimes);
% (adj res, dest res, time)Reservoir(r).Outflow = zeros(1, NumTimes);
% total effective outflow[veh / s] Reservoir(r).NinPerRoute =
    zeros(Temp_Nroutes, NumTimes);
% entry cumulative count per route Reservoir(r).NinPerResPerDest =
    zeros(NumRes, NumRes, NumTimes);
% nin(adj res, d, t) Reservoir(r).Nin = zeros(1, NumTimes);
% entry cumulative count[veh] Reservoir(r).NoutPerRoute = zeros(Temp_Nroutes,
                                                                NumTimes);
% exit cumulative count per route Reservoir(r).NoutPerResPerDest =
    zeros(NumRes, NumRes, NumTimes);
% nout(adj res, d, t) Reservoir(r).Nout = zeros(1, NumTimes);
% exit cumulative count[veh] if isfield (Reservoir, 'AccCircuPerRoute')
        Reservoir(r)
            .NoutCircuPerRoute = zeros(Temp_Nroutes, NumTimes);
% exit cumulative count of circulating veh per route
end
end
...
i_r = i_r + 1;
    ...
    % Mean speed Reservoir(r).MeanSpeedPerMode = Reservoir(r).MeanSpeed;
% mean speed per mode[m / s]

    % Inflow /
    outflow Reservoir(r).Inflow = sum(Reservoir(r).InflowPerRoute, 1);
...
for
  itime = 2 : NumTimes Reservoir(r).NinPerRoute(
                  :, itime) =
      Reservoir(r).NinPerRoute(
          :, itime - 1) +
      TimeStep * Reservoir(r).InflowPerRoute(
                     :, itime - 1);
Reservoir(r).NinPerResPerDest(
    :,
    :, itime) =
    Reservoir(r).NinPerResPerDest(
        :,
        :, itime - 1) +
    TimeStep * Reservoir(r).InflowPerResPerDest(
                   :,
                   :, itime - 1);
...
Reservoir(r).NoutPerRoute(
    ...
    Reservoir(r).NoutPerRoute(
        :, itime - 1) +
    TimeStep * Reservoir(r).OutflowPerRoute(
                   :, itime - 1);
