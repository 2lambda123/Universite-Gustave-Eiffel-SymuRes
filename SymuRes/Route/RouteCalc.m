...
for j = 1:
        length(ODmacro(od).Demand) % loop on all demand modes/purposes
...
if Temp_modeID == 0 % no mode/purpose found:
add a new mode
...
Route(iroute).TotalTime = sum(Temp_TT);
% route free-flow TT
...
% by default the time is discretized with the simul timestep
ODmacro(od).Demand(Temp_modeID).Data = ODmacro(od).Demand(Temp_modeID).Data + Temp_data;

end
Route = rmfield(Route,'Demand0');
% delete this field to save memory
end
...
% free flow travel times of the routes crossing the reservoir
Reservoir(r).RoutesID = [];
% routes ID crossing the reservoir
Reservoir(r).RoutesPathIndex = [];
% indexes of the reservoir in the paths
Reservoir(r).OriginRes = [];
% reservoir origins of the routes
Reservoir(r).DestinationRes = [];
% reservoir destinations of the routes
Reservoir(r).ODmacroID = cell(1,NumModes);
% ODmacro IDs of the routes

Reservoir(r).RoutesNodeID = [];
...
for iroute = 1:
        ...
        for r = Route(iroute).ResPath % loop on all reservoirs in the route
            ...
            % entry node ID
            inode2 = Route(iroute).NodePath(i_p+1);
            % exit node ID
