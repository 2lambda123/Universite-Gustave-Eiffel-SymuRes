...
% Calculation of the MSWA parameter at the current iteration iter =
    Assignment.CurIteration;
...
if
  ~isempty(Temp_RouteIDs) % Apply a choice model to calculate the path flow
                            distribution(
                                assignment coefficients) if Assignment.model <
      100 % Choice model ODflowdistrib = ChoiceModel(od, Temp_RouteIDs,
                                                     Reservoir, Route,
                                                     Assignment, Simulation);
else
  % Empirical model ODflowdistrib = EmpiricalModel(
      od, Temp_RouteIDs, Reservoir, Route, Assignment, Simulation);
end Temp_coeffs = ODflowdistrib(Temp_RouteIDs);
Temp_oldcoeffs = Assignment.OldAssignCoeff(Temp_RouteIDs);

% MSA step to update the assignment coefficients %
    Classical MSA Assignment.NewAssignCoeff(Temp_RouteIDs) =
    (1 - 1 / Assignment.CurIteration).*Temp_oldcoeffs +
    1 / Assignment.CurIteration.*Temp_coeffs;
% Weighted MSA(MSWA) Assignment.NewAssignCoeff(Temp_RouteIDs) =
    Temp_oldcoeffs + Temp_alphaMSWA.*(Temp_coeffs - Temp_oldcoeffs);
...
for j = 1:
        length(ODmacro(od).Demand)
            if strcmp(ODmacro(od).Demand(j).Purpose,'cartrip')
                Route(iroute).Demand = Route(iroute).AssignCoeff.*ODmacro(od).Demand(j).Data2;
% row vector end end %
    Append the number of vehicles assigned to each route Route(iroute)
        .NVehicles = ODmacro(od).NVehicules * Route(iroute).AssignCoeff;
end
end
end
...
for od = 1:
             ...
             Temp_demtot = 0;
for
  iroute = ODmacro(od).RoutesID Temp_demtot =
      Temp_demtot + sum(Route(iroute).Demand);
end
...
for
  od = 1 : NumODmacro Temp_coefftot = 0;
for
  iroute = ODmacro(od).RoutesID if Route (iroute).AssignCoeff <
           Temp_coeff_threshold Route(iroute).AssignCoeff = 0;
end Temp_coefftot = Temp_coefftot + Route(iroute).AssignCoeff;
end
for iroute = ODmacro(od).RoutesID
...
i_r = 1;
for
  iroute = Reservoir(r).RoutesID od = Route(iroute).ODmacroID;
...
else Reservoir(r).AvgTripLength(i_m, 1) = mean(Reservoir(r).TripLengthPerRoute);
end end end end
