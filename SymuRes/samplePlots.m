...
nModes = 2;
LW = 2;
cmap_perso = [51 51 255; 0 204 51; 204 0 0; 204 153 0; 153 0 102; 51 153 153;
                 204 102 204; 204 204 102] /
             255;
...
for
  ires = 1 : Nres modeindex = Reservoir(ires).ModeIndex{i_m};
subplot(1,nModes,i_m)
...
end plot(CurrentTime, demand, 'k', 'linewidth', LW)
end
end
...
legend('R1','R2')
    ...
    % hold on
% for i = 1:
                  length(Reservoir(1).InflowPerRoute(:,1))
...
% plot(DemTimesInterp, abs(DemTimesInterp - InflowTimesInterp)) % %
    plot(InflowCumulative(i,
                          :) /
             InflowCumulative(i, end),
         'color',
         cmap_perso(i,
                    :)) %
    %
    plot(DemCumulative(i,
                       :) /
             DemCumulative(i, end),
         '--', 'color',
         cmap_perso(i,
                    :)) %
    end % aa = sum(InflowCumulative, 1);
bb = sum(DemCumulative, 1);
% %
    plot(aa / aa(end), 'color',
         cmap_perso(4,
                    :)) %
    %
    plot(bb / bb(end), '--', 'color',
         cmap_perso(4,
                    :))
...
Pb = Pc;
for
  iline = 1 : size(kc, 1) k = [kc(iline, :); kb(iline, :)];
...
b2 = bbc + bcb;
b3 = uc;
% Critical acc. line for Global 3DMFD
bc1 = 2*bcc;
bc2 = bbc;
bc3 = uc;
% Critical acc. line for Car 3DMFD
bb1 = bcb;
bb2 = 2 * bbb;
bb3 = ub;
% Critical acc. line for Bus 3DMFD
ncline = [b1 b2 b3; ...
             bc1 bc2 bc3; ...
             bb1 bb2 bb3];
for
  ires = 1 : Nres modeindexc = Reservoir(ires).ModeIndex{1};
...
figure subplot(1, 2, 1) contour(kc, kb, Pc + Pb, 'ShowText', 'on');
hold on plot(-(ncline(1, 3) + ncline(1, 2) * [1:50]) / ncline(1, 1), [1:50],
             'k', 'LineWidth', LW) plot(nc, nb, 'linewidth', LW)
    colormap('jet');
colorbar subplot(1, 2, 2) contour(kc, kb, Pb, 'ShowText', 'on');
hold on plot(-(ncline(3, 3) + ncline(3, 2) * [1:50]) / ncline(3, 1), [1:50],
             'k', 'LineWidth', LW) plot(nc, nb, 'linewidth', LW)
    colormap('jet');
colorbar set(gcf, 'Position', [10 10 1400 2 * 300]) end
